...
_peco-gh-pull-request-url-pbcopy() {
  ...
  selected_pr=$(gh pr list --limit 300 --json number,title | jq -r '.[] | "\(.number) - [Pull Request] \(.title)"' | peco --select-1 2>/dev/null)
  ...
  cmd="gh pr view $pr_number --json url -q .url | pbcopy"
