...
EOF`
  ...
  print $cmd
  (ls $file_paths &>/dev/null && (eval $cmd $fail_fast_option $seed_option $tag_option $file_paths || true)) ||
    (ls $(git rev-parse --show-toplevel)/${^file_paths[*]} &>/dev/null && (eval $cmd $fail_fast_option $seed_option $tag_option $(git rev-parse --show-toplevel)/${^file_paths[*]} || true))
}
