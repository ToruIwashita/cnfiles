github-traffic() {
  ...
  for repo in $OWNED_GITHUB_REPOSITORIES; do
    curl -H "Authorization: token $GITHUB_API_TOKEN" https://api.github.com/users/$GITHUB_USER_NAME/repos 2>/dev/null | jq 'map(select(.name == "'$repo'")) | map({name, stargazers_count, open_issues_count}) | .[]'
    print 'views:'
