...
set -g status-right '#{prefix_highlight} #[fg=colour38,bg=colour234,nobold]#($TMUX_PLUGIN_MANAGER_PATH/tmux-mem-cpu-load/tmux-mem-cpu-load) #{?pane_synchronized,#[bg=colour54],#[bg=colour54]}#{?pane_synchronized,#[fg=colour231],#[fg=colour251,bg=colour242]} sync#{?pane_synchronized,[*],[]} #[default]#[fg=colour223,bg=colour166,nobold] [%Y-%m-%d(%a) %H:%M] '
