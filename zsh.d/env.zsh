...
export AI_AGENT_WITH_OPT="$AI_AGENT --chrome"           # デフォルトで使用されるAIエージェント（オプション付き）
export AI_AGENT_TMUX_SESSION='ai-agent-session:1.1'     # AIエージェントのTMUXセッション位置
export AI_SUB_AGENT=codex                               # デフォルトで使用されるAIサブエージェント
export AI_SUB_AGENT_WITH_OPT="$AI_SUB_AGENT"            # デフォルトで使用されるAIサブエージェント（オプション付き）
export AI_SUB_AGENT_TMUX_SESSION='ai-agent-session:1.0' # AIサブエージェントのTMUXセッション位置
...
export MEMOLIST_TMP_DUMP_PROMPT_FILE_PATH=$MEMOLIST_DIR_PATH/00-tmp-dump-prompt.md
