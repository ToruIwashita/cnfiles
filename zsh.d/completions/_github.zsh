...
__github-pr-numbers() {
  ...
  prs=(${(f)"$(gh pr list --limit 50 --json number,title,author -q '.[] | "\(.number) \(.title) @\(.author.login)"' 2>/dev/null)"})
...
__github-issue-numbers() {
  ...
  issues=(${(f)"$(gh issue list --limit 50 --json number,title,author -q '.[] | "\(.number) \(.title) @\(.author.login)"' 2>/dev/null)"})
